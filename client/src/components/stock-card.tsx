		...
		<>
			...
			>
				<div className='relative'>
					...
					/>
					<Badge
						className={`absolute top-3 right-3 ${status.className}`}
				...
				<div className='p-4'>
					<Tooltip>
						<TooltipTrigger asChild>
							<h3
								className='text-xl font-bold text-gray-900 leading-tight mb-1 truncate'
								data-testid={`text-stock-name-${stock.id}`}
							>
								{stock.name}
							</h3>
						</TooltipTrigger>
					...
					<p
						...
						data-testid={`text-stock-code-${stock.id}`}
					>
						Code: {stock.code}
					</p>
					<div className='flex justify-between items-center mb-4'>
						<span
							className={`text-2xl font-bold ${
								stock.quantity > 0 ? 'text-primary' : 'text-muted-foreground'
							}`}
