...
const requestSchema = (stocks: Stock[]) =>
	z
		...
		.refine(
			(data) => {
				const selected = stocks.find((s) => s.id === data.stockId);
				console.log(selected, data.quantity);
				return !selected || data.quantity <= selected.quantity;
			},
			{
...
type RequestFormData = z.infer<ReturnType<typeof requestSchema>>;
